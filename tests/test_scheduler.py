...
jobs:
  - name: job_name
    ...
    def test_no_job_project_report(self):
        ...
        self.config.set('zuul', 'layout_config',
                        'tests/fixtures/layout-no-job-report.yaml')
        self.sched.reconfigure(self.config)
        self.registerJobs()
        A = self.fake_gerrit.addFakeChange('org/no-jobs-project',
                                           'master', 'A')
...
For CI problems and help debugging, contact ci@example.org"""
    ...
    def test_layout_abort_pipelines(self):
        ...
        "manager other than IndependentPipelineManager raises error"
        self.config.set('zuul', 'layout_config',
                        'tests/fixtures/layout-bad-abort-pipelines.yaml')
